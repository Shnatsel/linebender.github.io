...
Fearless SIMD is here to stay. We will be providing [3 years of security updates](https://github.com/linebender/fearless_simd/blob/main/fearless_simd/SECURITY.md) for v1.0 and all later versions.

While we cannot see the future, there are viable paths to supporting both near-term Rust features, such as the `f16` type, and longer-term features such as [SVE](https://en.wikipedia.org/wiki/AArch64#Scalable_Vector_Extension_(SVE)) and [RISC-V Vector Extension](https://rvv-isadoc.readthedocs.io/en/latest/index.html) if/when these hardware extensions become relevant, without API-breaking changes.
